...
const VoteButton = ({ proposalId }: Props) => {
  ...
  if (vote) {
    return <FixedButton variant="outlined" disabled>{vote.support === VoteSupport.For ? "For" : "Against"}</FixedButton>;
  }

  return <Stack direction="row" justifyContent="flex-end" spacing={1}>
    <FixedButton variant="contained" color="success" onClick={() => executeVote(VoteSupport.For)}>For</FixedButton>
    <FixedButton variant="outlined" color="error" onClick={() => executeVote(VoteSupport.Abstain)}>Against</FixedButton>
  </Stack>;
};
