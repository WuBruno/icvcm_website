...
const AccountInfo = (props: Props) => {
  ...
  const { data } = useSWR(account ? "member" : null, async () => getMember(ICVCMRoles, account));
  ...
  return (
    <Stack sx={{ marginY: 2 }}>
      <Typography>
        <Typography fontWeight="bold">Name:</Typography> {ethers.utils.parseBytes32String(data.name)}
      </Typography>
      <Typography>
